...
get_latest_activity_timestamp() {
	...
	latest_bot_reminder_timestamp=$(echo "$comments_json" | jq -r '
		map(select(.body | (contains("<!-- PR_INACTIVITY_REMINDER -->") or contains("<!-- PR_2_HOUR_REMINDER -->")))) |
	...
	if [[ -z "$latest_human_activity_timestamp" ]]; then
		...
		latest_comment_updated_at=$(echo "$comments_json" | jq -r '
			map(select(.body | (contains("<!-- PR_INACTIVITY_REMINDER -->") or contains("<!-- PR_2_HOUR_REMINDER -->")) | not)) |
			map(select(.createdAt | type == "string")) |
			map(.createdAt) |
			max // null
		')
...
process_pr() {
	...
	if ((inactive_days >= DAYS_TO_CLOSE)) && [[ "$reminder_count" -eq 2 ]]; then
		...
		if [[ -z "$oncaller" || "$oncaller" == "null" ]]; then
			oncaller="Neelabh94"
		fi
		oncaller="@${oncaller}"
		...
		if [[ "$approval_status_code" -eq 0 ]]; then
			...
			gh pr comment "$pr_number" --body "This PR has been inactive for 30 seconds. ${oncaller}, please review. <!-- PR_2_HOUR_REMINDER -->"
		fi
	else
