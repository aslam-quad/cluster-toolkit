...
OWNER="${GITHUB_OWNER:-aslam-quad}"
REPO_NAME="${GITHUB_REPO_NAME:-cluster-toolkit}"
REPO="${OWNER}/${REPO_NAME}"

OLD_TAG=$(gh release list -R "${REPO}" -L 1 --json tagName --jq '.[] | .tagName')
...
echo "RC_BRANCH_NAME:${RC_BRANCH}"

gh repo clone "${REPO}" "${GITDIR}" -- --single-branch --branch develop --depth 1 --origin "${REMOTE_NAME}"
cd "${GITDIR}"
git remote set-url "${REMOTE_NAME}" "https://${GITHUB_TOKEN}@github.com/${REPO}.git"
...
PR_URL=$(gh pr create --repo "${REPO}" --base "${RC_BRANCH}" --head "${V_BRANCH}" \
	--title "Update Toolkit release to ${NEW_TAG}" \
	--body "Set release-candidate to version ${NEW_VERSION}" || gh pr list --repo "${REPO}" --state open --head "${V_BRANCH}" --base "${RC_BRANCH}" --json url --jq '.[0].url')
echo "$PR_URL"
echo
echo
echo
